...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if (!message.member.voice.channel) return message.channel.send("Veuillez rejoindre un salon vocal ou transférez le membre vers.")



        membre.voice.setChannel(message.member.voice.channel).then(() => {
            message.channel.send(`Je viens de le move dans votre salon vocal.`)
        })
    }
}